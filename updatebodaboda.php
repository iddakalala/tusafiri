...
if(empty ($plateNo) )
{
    $status="Andika namba ya bodaboda";
    ...
    //PREPARE SQL COMMAND
   $query = "SELECT `plate_number` FROM `bodaboda` WHERE `plate_number`='$plateNo'";
  ...
  if($query_run= mysql_query($query))
  	
     {
     	...
     	if($query_num_rows==0)
     	{
     		$status= "<span style='color:red;'>namba ya bodaboda uliyoingiza si sahihi</span>";
     	}else{
                 		$sql="SELECT * from  bodaboda WHERE `plate_number`='$plateNo'";
 ...
 $query="UPDATE bodaboda SET bodaboda_name='$jina',plate_number='$plateNo',route_source='$source',route_destination='$destination',route_cost='$cost',route_status='$statusi' WHERE plate_number='$plateNo'";
...
<form action="updatebodaboda.php" method="post">

<div id="search" style="margin:20px;">
	<label for="bno">Namba ya bodaboda:</label>
   ...
   <form action="" method="post" >
	            <table class="table table-condensed">
										...
										<tr>
									    <td><label for="plateNo">Namba ya Bodaboda:</label></td>
									   ...
									   <td> <input type="text" name="contact" id="contact"  value="<?php echo $contact; ?>" required></td>
										...
										<a href="bodaboda.php" class="btn btn-primary">Rudi</a>
										</td>
										</tr>
	            </table>
