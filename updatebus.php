...
if(empty ($platenumber) )
{
    ...
    //PREPARE SQL COMMAND
   $query = "SELECT `platenumber` FROM `bus` WHERE `platenumber`='$platenumber'";
  ...
  if($query_run= mysql_query($query))
  	
     {
     	...
     	if($query_num_rows==0)
     	{
     		$status= "<span style='color:red;'>namba ya basi uliyoingiza si sahihi</span>";
     	}else{
                 		$sql="SELECT * from  bus,company WHERE (bus.company_id=company.company_id) AND `platenumber`='$platenumber'";
 ...
 $query="UPDATE bus,company SET bus_name='$jina',platenumber='$plateNo',route_source='$source',route_destination='$destination',route_via='$via',route_cost='$cost',route_status='$statusi',company.company_id='$kampuninumber',company.company_name='$kampuni',company.company_contact='$contact' WHERE (bus.company_id=company.company_id) AND platenumber='$plateNo'";
...
<div id="search" style="margin:20px;">
	<label for="bno">Namba ya bus:</label>
	            ...
	            <table class="table table-condensed">
						            <tr>
									   ...
									   <td> <input type="text" name="contact" id="contact"  value="<?php echo $contact; ?>" required></td>
									    ...
									    <tr>
										<td><label for="jina">Jina la Bus:</label></td>
										<td><input type="text" name="jina" id="jina" value="<?php echo $jina; ?>" required></td>
										</tr>
										<tr>
									    <td><label for="plateNo">Namba ya Bus:</label></td>
									   ...
									   <td><label for="destination">Kwenda:</label></td>
										...
										<a href="bus.php" class="btn btn-primary">Rudi</a>
										</td>
										</tr>
	            </table>
